import { ADMIN_BUILDINGS, START_COINS } from '../config/constants.js';
import { cardinalRoads, mooreNeighborhood, plotKey, computePlotCost } from '../utils/grid.js';

const defaultPlayer = (id, name) => ({
  id,
  name,
  coins: START_COINS,
  crystals: 0,
...
export class CityState {
  constructor() {
    ...
    this.plots = new Map();
    this.roads = new Map();
    this.seedStarterBuildings();
    this.seedInitialBuyablePlots();
  }

  seedStarterBuildings() {
    this.buildings = ADMIN_BUILDINGS.map((building, index) => ({
      id: `admin-${index}`,
      ownerId: 'admin',
  ...
  registerPlayer(id, name = `Guest-${id.slice(-4)}`) {
    const player = defaultPlayer(id, name);
